...
Racmacs.Viewer.prototype.showLegend = function () {
	...
	ag_group_levels.map((level, i) => {
		if (ag_group_values.indexOf(i) != -1) {
			...
			var num_ags_in_group = ag_group_values.filter(x => x == i).length;
			var legend_text = level + " (" + num_ags_in_group + ")";
	...
	sr_group_levels.map((level, i) => {
		if (sr_group_values.indexOf(i) != -1) {
			...
			var num_srs_in_group = sr_group_values.filter(x => x == i).length;
			var legend_text = level + " (" + num_srs_in_group + ")";
