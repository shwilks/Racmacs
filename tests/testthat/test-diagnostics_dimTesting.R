...
test_that("Dimension test summary working correctly", {
  ...
  expect_equal(
    mean(c(
      abs(predtiter1 - log_titers(testtiter1, 1)),
      abs(predtiter2 - log_titers(testtiter2, 1))
    )),
...
test_that("Dimension testing", {
  ...
  lapply(results, function(result) {
    ...
    table_dists <- matrix(
      colbases,
      nrow(log_titer_table),
      ncol(log_titer_table),
      byrow = T
    ) -
      log_titer_table
    ...
    for (x in seq_along(result$dim)) {
      ...
      agBaseCoords(testmap) <- result$coords[[x]][
        seq_len(numAntigens(map)),
        ,
        drop = F
      ]
      srBaseCoords(testmap) <- result$coords[[x]][
        -seq_len(numAntigens(map)),
        ,
        drop = F
      ]
      ...
      agsrdists <- as.matrix(dist(result$coords[[x]]))[
        seq_len(numAntigens(map)),
        -seq_len(numAntigens(map))
      ]
