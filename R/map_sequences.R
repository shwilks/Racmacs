...
get_pts_sequence_matrix <- function(points, missing_value) {
  rbind_list_to_matrix(
    lapply(points, function(pt) {
      ...
      seq_insertion_positions <- vapply(
        pt$sequence_insertions,
        function(x) x[[1]],
        numeric(1)
      )
      seq_insertions <- vapply(
        pt$sequence_insertions,
        function(x) x[[2]],
        character(1)
      )
...
set_pts_sequence_matrix <- function(points, seq_matrix) {
  ...
  for (i in seq_along(points)) {
    ...
    pt_insertions <- substr(
      pt_sequence[pt_insertion_positions],
      2,
      nchar(pt_sequence[pt_insertion_positions])
    )

    # Remove insertions from main sequence
    pt_sequence[pt_insertion_positions] <- substr(
      pt_sequence[pt_insertion_positions],
      1,
      1
    )
    ...
    points[[i]]$sequence_insertions <- lapply(
      seq_along(pt_insertion_positions),
      function(i) {
        list(
