...
test_map_subset <- function(map, ag_subset, sr_subset) {
  ...
  expect_equal(
    agReactivityAdjustments(map_subset),
    agReactivityAdjustments(map)[ag_subset]
  )
  ...
  for (x in seq_len(numOptimizations(map))) {
    expect_equal(
      agCoords(map_subset, x),
      agCoords(map, x)[ag_subset, , drop = F]
    )
    expect_equal(
      srCoords(map_subset, x),
      srCoords(map, x)[sr_subset, , drop = F]
    )
  }
  ...
  expect_equal(
    ...
    rank(
      ptDrawingOrder(map)[c(ag_subset, sr_subset + numAntigens(map))],
      ties.method = "first"
    )
  )
}
