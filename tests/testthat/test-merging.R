...
titer_merge_tests <- tibble::tribble(
  ...
  "10",
  "10",
  "10", # numeric unchanged
  c("<10"),
  "<10",
  "<10",
  "<10",
  "<10",
  "<10", # less than unchanged
  c(">80"),
  ">80",
  ">80",
  ">80",
  ">80",
  ">80", # greater than unchanged
  c("."),
  ".",
  ".",
  ".",
  ...
  "10",
  "10",
  "10", # two same numeric, merges same
  c("*", "10", "10", "*"),
  "10",
  "10",
  "10",
  "10",
  "10", # two same numeric, two *,  merges same
  c("<10", "<40", "<20", "*"),
  "<10",
  "<10",
  "<10",
  "<10",
  "<10", # all less than go to min
  c(">10", ">40", ">20", "*"),
  ">40",
  ">40",
  ">40",
  ">40",
  ">40", # all greater than to to max
  c("10", "160", "2560", "*"),
  "160",
  "160",
  "*",
  "*",
  "*", # very variable not set to . (default sd_limit=NA), including *
  c("<10", "20", "*", "*"),
  "<40",
  "10",
  "<40",
  "*",
  "*", # mix of numeric & less than
  c("<10", "20", "*", "."),
  "<40",
  "10",
  "<40",
  ...
  "*",
  "*",
  "*", # mix of numeric, less than, and greater than, becomes *
  c("<20", "40", ">10", "."),
  "*",
  "*",
  "*",
  "*",
  "*", # mix of numeric, less than, greater than and ., becomes *
  c("<10", "320"),
  "<640",
  "40",
  "*",
  "*",
  "*", # variable mix of < and numeric
)
...
test_that("Test titer merging", {
  for (x in seq_len(nrow(titer_merge_tests))) {
    ...
    expect_equal(
      suppressWarnings(ac_merge_titers(
        titer_merge_tests$titers[[x]],
    ...
    expect_equal(
      ac_merge_titers(
        titer_merge_tests$titers[[x]],
        options = RacMerge.options(method = "conservative")
      ),
      titer_merge_tests$conservative[x]
    )
    ...
    expect_equal(
      ac_merge_titers(
        titer_merge_tests$titers[[x]],
        options = RacMerge.options(method = "likelihood")
      ),
    ...
    expect_equal(
      ac_merge_titers(
        titer_merge_tests$titers[[x]],
        options = RacMerge.options(method = "lispmds")
      ),
    ...
    expect_equal(
      ac_merge_titers(
        titer_merge_tests$titers[[x]],
        options = RacMerge.options(method = "conservative", sd_limit = 1)
      ),
    ...
    expect_equal(
      ac_merge_titers(
        titer_merge_tests$titers[[x]],
        options = RacMerge.options(method = "likelihood", sd_limit = 1)
      ),
...
test_that("Merge sd_lim working", {
  ...
  expect_equal(
    "*",
    ac_merge_titers(
      c(rep("<10", 50), rep("20", 50), "40"),
      options = RacMerge.options(sd_limit = 1, method = "likelihood")
    )
  )
  ...
  expect_equal(
    ...
    ac_merge_titers(
      c("<10", "20"),
      options = RacMerge.options(sd_limit = 0.8, method = "likelihood")
    )
  )
})
